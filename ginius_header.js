...
document.addEventListener("DOMContentLoaded", function () {
  const offcanvasMenuItems = document.querySelectorAll(
    ".ofcanva-menu .elementor-icon-list-items",
  );
...
.dex_header:hover{
 ...
 .dex_header:hover span,
.dex_header:hover .wp-image-13378,
.dex_header:hover li,
.dex_header:hover svg
{
    filter: brightness(0);
}
...
document
  .querySelectorAll(".head-menue-item .menu-item")
  .forEach((item, index) => {
...
document
  .querySelectorAll(".sub-menu-item .elementor-icon-list-item")
  .forEach((item, index) => {
    item.addEventListener("mouseover", () => {
      removeSubActive();
...
document
  .querySelectorAll(".sub-menu-item2 .elementor-icon-list-item")
  .forEach((item, index) => {
    item.addEventListener("mouseover", () => {
      removeSubActive2();
...
document
  .querySelectorAll(".sub-menu-item3 .elementor-icon-list-item")
  .forEach((item, index) => {
    item.addEventListener("mouseover", () => {
      removeSubActive3();
...
window.addEventListener("scroll", function () {
  ...
  // Reset background when at very top
  if (scrollTop === 0) {
    if (header) {
      header.style.backgroundColor = "transparent";
      header.style.transform = "translateY(0)";
    }
    if (mobHeader) mobHeader.style.backgroundColor = "transparent";
  } else {
    if (header) header.style.backgroundColor = "#00000070";
    if (mobHeader) mobHeader.style.backgroundColor = "#00000070";
  }
...
const menuItems = document.querySelectorAll(
  ".main_item-menu .elementor-nav-menu--main .menu-item",
);
...
window.addEventListener("mouseover", (e) => {
  if (
    !e.target.closest(".main_item-menu") &&
    !e.target.closest(".main_sub-menu")
  ) {
    menuSubContent.forEach((sub) => {
      sub.style.transform = "translateY(30px)";
      sub.style.opacity = "0";
